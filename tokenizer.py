...
and only accept the ones that seem to result in good diffs, it's a sort of
...
dash = r'''([0-9a-zA-Z]+\-){1,}[0-9A-Za-z]+'''

assert '-' != '―'
Separators = regex_or('--+', '―')
Decorations = r' [  ♫   ]+ '.replace(' ', '')

EmbeddedApostrophe = r"\S+'\w+"   # r"\S+'\S+"
...
#   if isinstance(s,unicode): return s.decode(encoding, *args)
#   return unicode(s)
...
def tokenize(tweet):
    ...
    text = re.sub(Url_RE, "HTTPURL", text)
    text = squeeze_whitespace(text)

    text = re.sub(r'[^\x00-\x8f]', '', text)
...
AposS = mycompile(r"(\S+)(['’][sS])$")
split_dash = mycompile(r"-")
...
WS_RE = mycompile(r'\s+|/|~')
REGR_APOST = mycompile(r"""['’]""")


def squeeze_whitespace(s):
    new_string = WS_RE.sub(" ", s)
    new_string = REGR_APOST.sub("'", new_string)
...
if __name__ == '__main__':
    for line in sys.stdin:
        # line = ""
        # print(u" ".join(tokenize(line[:-1])).encode('utf8'))
        print(line.encode('utf8'))
        s = " ".join(tokenize(line[:-1]))
        print(re.sub(r'[^\x00-\xf3]', '', s))
        # print(u" ".join(x for x in tokenize(line[:-1]) if ord(x) < 256))
